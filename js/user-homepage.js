...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (apiDocElement) {
        apiDocElement.addEventListener('click', (event) => {
            ...
            xhr.open('GET', 'https://isa-project-backend-ultkx.ondigitalocean.app/api-docs'); // Replace with your actual URL

            xhr.onreadystatechange = function() {
                if (xhr.readyState === XMLHttpRequest.DONE) { // Request is complete
                    if (xhr.status >= 200 && xhr.status < 300) { // Success
                        window.location.href = 'https://isa-project-backend-ultkx.ondigitalocean.app/api-docs';
                    } else { // Server error
